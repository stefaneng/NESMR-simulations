...
create_and_save_plots <- function(pvals_df, pvalue_col, method_name, output_dir) {
    # Histogram
    pvalue_hist <- ggplot(pvals_df) +
        geom_histogram(aes(x = get(pvalue_col), after_stat(density)), breaks = seq(0, 1, by = 0.1)) +
        theme_minimal() +
        xlab(sprintf("%s P-values", method_name)) +
        theme(text = element_text(size = 24))

    ggsave(
        filename = sprintf("%s/%s_pvalue_hist_dsc.jpg", output_dir, method_name),
    ...
    pvalue_qqplot_log <- ggplot(pvals_df, aes(sample = -log10(get(pvalue_col)))) +
        ...
        ylab(expression(atop(method_name, paste("(", -log[10], " p-value)")))) +
        theme_minimal() +
        theme(text = element_text(size = 24))

    ggsave(
        filename = sprintf("%s/%s_pvalue_qqplot_log_dsc.jpg", output_dir, method_name),
