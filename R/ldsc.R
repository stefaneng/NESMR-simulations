# Copied and modified for use in GWASBrewer
# From: https://github.com/GenomicSEM/GenomicSEM/blob/master/R/ldsc.R
# N is the number of individuals in the sample (for now assume that each traits has same N)

.LOG <- function(..., file = NULL, print = TRUE) {
  msg <- paste0(..., "\n")
  if (print) cat(msg)
  if (! is.null(file)) cat(msg, file = file, append = TRUE)
  else cat(msg)
}

ldsc_GWASBrewer <- function(
    dat, N, n.blocks = 200, chisq.max = NA, stand = FALSE, log.file = NULL, trait.names = NULL) {
  time <- proc.time()
  begin.time <- Sys.time()

  n.traits <- ncol(dat$beta_hat)
  traits <- 1:n.traits
  sample.prev <- rep(NA, n.traits)
  population.prev <- rep(NA, n.traits)
  ...
  # `x` matrix is the LD scores
  x <- data.frame(
    CHR = 19,
    SNP = dat$snp_info$SNP,
    BP = 1:nrow(dat$snp_info),
    L2 = dat$snp_info$l2
  )

  # Rename as weights..
  # Just keeping here in case we want different weights eventually?
  w <- x
  colnames(w)[ncol(w)] <- "wLD"

  # We assume that all SNPs are > 5% MAF so each trait is just >
  m <- M.tot <- nrow(dat$snp_info)

  Z_mat <- with(dat, beta_hat/s_estimate)

  all_y <- lapply(traits, function(j) {
    data.frame(
      SNP = dat$snp_info$SNP,
      N = N,
      Z = Z_mat[, j],
