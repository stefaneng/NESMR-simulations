...
if (alpha < 1) {
  ...
  ma_models <- lapply(eta, function(.eta) {
    ...
    ma_adj_dat$beta_hat[rand_ix] <- unbias_SNPs$beta_rb[valid_ix]
    ma_adj_dat$s_estimate[rand_ix] <- unbias_SNPs$se_rb[valid_ix]
  ...
  ma_mod_results <- lapply(seq_along(eta), function(i) {
    cbind.data.frame(
