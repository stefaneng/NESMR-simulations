...
correct_edges <- cbind(
  read.csv('correct_edges.csv'),
  winners_curse = TRUE)
correct_edges_no_LD <- cbind(
  read.csv('correct_edges_no_LD.csv'),
  winners_curse = FALSE)
...
levels(correct_edges$Result) <- levels(correct_edges_no_LD$Result) <- c('True Positive', 'Extra Edge', 'Missing Edge', 'True Remove')
...
kable_correct <- select(correct_edges, -KeepEdge.Actual, -winners_curse) %>%
  relocate(method) %>%
  relocate(Result)
...
The edges that appear the most are 5->1 and 4->1.
These are both edges that have an indirect path (5->2->1 and 4->2->1) so this could affect it.
My first though was to confirm that each of these extra edges has correct p-value distribution outside of the backward selection algorithm

## Extra Edge p-value simulation

1. Simulate from the true graph
2. Test each of the extra possible edges on lower triangular
  - Use LRT and Delta Method p-values
3. Check calibration of p-values < 0.05  
3. Plot (log) p-values and qq-plots to assess

## P-value calibration

| row | col | LRT mean(p-val < 0.05) | DM mean(p-val < 0.05) |
|-----|-----|------------------------|-----------------------|
| 3   | 1   | 0.126                  | 0.08                  |
| 3   | 2   | 0.052                  | 0.114                 |
| 4   | 1   | 0.45                   | 0.202                 |
...
| edge | LRT mean(p-val < 0.05) | DM mean(p-val < 0.05) |
|--- |------------------------|-----------------------|
| 3 -> 1   | 0.126                  | 0.08                  |
| 3 -> 2   | 0.052                  | 0.114                 |
| 4 -> 1   | 0.45                   | 0.202                 |
...
<!-- ```{r, results='asis'} -->
<!-- # Run this then copy paste -->
<!-- for(e in apply(extra_edges, 1, paste0, collapse = '_')) { -->
<!--   cat(sprintf('## Edge: %1$s  -->
<!-- :::: {.columns} -->

<!-- ::: {.column width="50%%"} -->
<!--   ![](figures/2024-01-11_delta_method_%1$s_pvalue_hist_dsc.jpg) -->
<!-- ::: -->

<!-- ::: {.column width="50%%"} -->
<!--   ![](figures/2024-01-11_lrt_%1$s_pvalue_hist_dsc.jpg) -->
<!-- ::: -->
...
<!-- ::: {.column width="50%%"} -->
<!--   ![](figures/2024-01-11_delta_method_%1$s_pvalue_qqplot_log_dsc.jpg) -->
<!-- ::: -->

<!-- ::: {.column width="50%%"} -->
<!--   ![](figures/2024-01-11_lrt_%1$s_pvalue_qqplot_log_dsc.jpg) -->
<!-- ::: -->
